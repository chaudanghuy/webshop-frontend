...
const ChooseListings = ({ visible, setVisible, listings, selectedShops, selectedTemplate }) => {
  ...
  const editListing = (listingId) => {
    ...
    setSelectedListings(
      selectedListings.map((l) => (l.id === listingId ? { ...l, isEdit: true } : l)),
    )
  ...
  const processEditListing = (listingId, listingNameEdit) => {
    setSelectedListings(
      selectedListings.map((l) => (l.id === listingId ? { ...l, name: listingNameEdit } : l)),
    )
  }

  const finishEditListing = (listingId) => {
    setSelectedListings(
      selectedListings.map((l) => (l.id === listingId ? { ...l, isEdit: false } : l)),
    )
  ...
  return (
    <div className="app">
            ...
            <div className="column input-section">
              ...
              <CRow className="mt-3">
                <CFormLabel className="col-4" controlId="exampleForm.ControlInput1">
                  Shop:{' '}
                  {step2Shops &&
                    step2Shops.map((shop, index) => (
                      <CBadge color="danger" key={index}>
                        {shop.name}
                      </CBadge>
                    ))}
                </CFormLabel>
            ...
            <div className="column product-list">
              <div className="header-fixed d-flex justify-content-between align-items-center">
                ...
                <div className="d-flex align-items-center ms-auto">
                  <CFormLabel htmlFor="isDraft" className="me-2 mb-0">
                    Đăng Draft
                  </CFormLabel>
                  <Toggle
                        ...
                        <CTableRow key={index}>
                          ...
                          <CTableDataCell>
                            {listing.isEdit ? (
                              <CFormTextarea
                                name="name"
                                value={listing.name}
                                onChange={(e) => processEditListing(listing.id, e.target.value)}
                              />
                            ) : (
                          ...
                          <CTableDataCell>
                            <CRow className="d-flex flex-column">
                              <CCol>
                                {listing.isEdit ? (
                                  <CButton
                                    color="success"
                                    className="me-2 mb-2"
                                    onClick={() => finishEditListing(listing.id, listing.name)}
                                  >
                              ...
                              <CCol>
                                {listing.isCertUpload == 0 ? (
                                  <CButton
                                    color="warning"
                                    onClick={() => showUploadCertModal(listing.id)}
                                  >
                                    <CIcon icon={cilLockLocked} className="me-2" /> Chứng chỉ
                                  </CButton>
                                ) : (
                                  <CBadge color="success">Đã up cert</CBadge>
                                )}
                              </CCol>
                            </CRow>
                          </CTableDataCell>
            ...
            {isUploading ? (
              ...
              <>
                <CButton
                  color="primary"
                  className="me-5"
                  onClick={uploadTiktokProducts}
                  disabled={uploadBtnStatus ? true : false}
                >
                  Đăng sản phẩm
                </CButton>
                <CFormLabel htmlFor="isCheckStatus" className="me-2 mb-0">
                  Check Listing
                </CFormLabel>
                <Toggle
      ...
      <UploadCertModal
        visible={visibleUploadCert}
        setVisible={setVisibleUploadCert}
        listingCert={listingNeedCert}
      />
    </div>
  )
}
    ...
    <div>
      ...
      >
        ...
        <CModalBody className="d-flex flex-column">
          <CRow>
            <CFormLabel htmlFor="listingName" className="me-2 mb-0">
              Sản phẩm {listingCert && listingCert.name}
            </CFormLabel>
          </CRow>
          <CRow>
            <CFormLabel>Upload Certificate</CFormLabel>
            {listingCert && certImages.length > 0 ? (
