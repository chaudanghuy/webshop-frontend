...
const AddTemplate = () => {
  ...
  const handleFormSKUTempChange = (fieldKey, value) => {
    ...
    const isExisting =
      Array.isArray(skuTempAttribute) && skuTempAttribute.some((item) => item.id === fieldKey)
    if (isExisting) {
  ...
  const handleSubmit = async (e) => {
    ...
    // define sku because it can be empty
    let finalSkus = []
    if (formSkus.length > 0) {
      finalSkus = formSkus.map((sku) => sku.sku)
    } else {
      sku.forEach((sku) => {
        finalSkus.push({
          // generate random id
          ...
          // random image
          image:
            'https://res.cloudinary.com/dg5multm4/image/upload/v1736698626/posts/hkg8lurioxouyp1gok1f.jpg',
          price: sellerSkuPrice,
      ...
      <CForm method="post" onSubmit={handleSubmit}>
        <CRow>
          <CCol xs={12}>
            <CCard>
              <CCardHeader>
                <strong>THÔNG TIN TEMPLATE</strong>
              </CCardHeader>
              <CCardBody>
                ...
                <div className="row g-3">
                  {attributes.map((field) => (
                    <DropdownSearch
                      ...
                      value=""
                      onChange={(data) =>
                        handleFormAttributeChange(data.id, data.value, data.label)
                      }
                      style="col-4"
                    />
                  ))}
                ...
                <div className="row g-3">
                  {compliances.map((compliance, index) => (
                    <CCol md={4} key={index}>
                      ...
                      <CFormSelect
                        aria-label="Product Compliance"
                        onChange={(e) =>
                          handleFormComplianceChange(
                            compliance.id,
                            e.target.value,
                      ...
                      >
                        ...
                        {compliance.options &&
                          compliance.options.map((value, index) => (
                      ...
                      <div key={sku.id}>
                        <CRow className="mt-2">
                          ...
                          <CCol md={3}>
                            <CFormInput
                              type="text"
                              onChange={(e) => handleFormSKUTempChange(sku.id, e.target.value)}
                              label="Giá trị"
                            />
                          </CCol>
                          <CCol md={2}>
                            <CButton
                              className="mt-4 ms-5 circle float-start"
                              color="warning"
                    ...
                    <CCol col={3}>
                      <CInputGroup className="mb-3">
                        <CDropdown variant="input-group">
                          ...
                          <CDropdownMenu>
                            {identifierCodes.map((code, index) => (
                              <CDropdownItem
                                key={index}
                                onClick={() => handleIdentifierCodeChange(code)}
                              >
                                {code}
                              </CDropdownItem>
                            ))}
                        ...
                        <CFormInput
                          ...
                          onChange={(e) => setIdentifierCodeValue(e.target.value)}
                    ...
                    <CCol col={3}>
                      <CInputGroup className="mb-3">
                        ...
                        <CFormInput
                          ...
                          id="skuPrice"
                          name="skuPrice"
                          onChange={(e) => setSellerSkuPrice(e.target.value)}
        ...
        <CRow className="mt-3">
          <CFormLabel color="danger" className="text-danger">
            {errorMessage}
          </CFormLabel>
        </CRow>
      </CForm>
