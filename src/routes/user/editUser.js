...
const EditUser = ({ visible, setVisible, user }) => {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        const teams = await apiRequest.get('/teams').then((res) => {
          setTeams(res.data.teams)
          const shops = apiRequest.get(`/shops`).then((res2) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        shops: JSON.stringify(selectedShops.map((shop) => shop.id)),
      }
    ...
    >
      ...
      <CModalBody>
        <CRow className="mt-3 col-3">
          <CRow className="d-flex justify-content-center">
            <CImage
              ...
              className="col-12 img-fluid mb-2"
            />
        ...
        <CForm className="row g-3" method="post" onSubmit={handleSubmit}>
          ...
          <CRow className="mt-3">
            <CCol md={12}>
              <CFormLabel>Shop quản lý</CFormLabel>
              <MultiSelect
                displayValue="name"
                options={shops}
                onSelect={searchBy}
